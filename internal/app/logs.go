...
var (
	...
	// Bunker status - live embed (edit instead of send new)
	bunkerMessageID  string
	bunkerMsgMux     sync.RWMutex
	lastBunkerHash   string
)
...
type BunkerLog struct {
	Date        time.Time `json:"date"`
	BunkerName  string    `json:"bunker_name"`
	Status      string    `json:"status"`
	ActivatedAgo string   `json:"activated_ago"`
	Location    Location  `json:"location"`
}
...
func InitializeLogs() error {
	...
	// Load bunker message ID for live status updates
	loadBunkerMessageID()
...
func parseEconomyLog(line string) *EconomyLog {
	...
	// Pattern 1: SOLD format (มี health field) - รองรับทั้งที่มีและไม่มี uses/stack count field
	soldPattern := `(\d{4}\.\d{2}\.\d{2}-\d{2}\.\d{2}\.\d{2}): \[(Trade|Purchase)\] Tradeable \((.+?) \(health: ([\d.]+)(?:, (?:uses|stack count): (\d+))?\)\) (sold|bought) by (.+?)\((\d+)\) for (\d+) \((\d+) \+ \d+ worth of contained items\) to trader (.+?), old amount in store is ([\d-]+), new amount is ([\d-]+)`
...
func parseBunkerLog(line string) *BunkerLog {
	if line == "" || !strings.Contains(line, "[LogBunkerLock]") {
		return nil
	}

	// Pattern: status line with coordinates
	// e.g. "2026.03.15-05.01.44: [LogBunkerLock] D1 Bunker is Active. Activated 00h 00m 00s ago. X=-537889.562 Y=540004.312 Z=81279.648"
	statusPattern := `(\d{4}\.\d{2}\.\d{2}-\d{2}\.\d{2}\.\d{2}): \[LogBunkerLock\] (.+?) Bunker is (Active|Locked)\. Activated (\d+h \d+m \d+s) ago\. X=([\d.-]+) Y=([\d.-]+) Z=([\d.-]+)`
	...
	if len(matches) == 8 {
		...
		y, _ := strconv.ParseFloat(matches[6], 64)
		z, _ := strconv.ParseFloat(matches[7], 64)

		return &BunkerLog{
			Date:         date.UTC(),
			BunkerName:   matches[2],
			Status:       matches[3],
			ActivatedAgo: matches[4],
			Location: Location{
				X: x,
				Y: y,
				Z: z,
	...
	// Pattern: activation notification (no coordinates)
	// e.g. "2026.03.15-05.01.35: [LogBunkerLock] D1 Bunker Activated 23h 18m 30s ago"
	activationPattern := `(\d{4}\.\d{2}\.\d{2}-\d{2}\.\d{2}\.\d{2}): \[LogBunkerLock\] (.+?) Bunker Activated (\d+h \d+m \d+s) ago`
	re = regexp.MustCompile(activationPattern)
	matches = re.FindStringSubmatch(line)
...
// loadBunkerMessageID loads the saved message ID from disk
func loadBunkerMessageID() {
	data, err := os.ReadFile("bunker_message_id.txt")
	if err == nil {
		bunkerMsgMux.Lock()
		bunkerMessageID = strings.TrimSpace(string(data))
		bunkerMsgMux.Unlock()
		if bunkerMessageID != "" {
			log.Printf("[BUNKER] Loaded message ID: %s", bunkerMessageID)
		}
	}
}

// saveBunkerMessageID saves the message ID to disk
func saveBunkerMessageID(id string) {
	bunkerMsgMux.Lock()
	bunkerMessageID = id
	bunkerMsgMux.Unlock()
	os.WriteFile("bunker_message_id.txt", []byte(id), 0644)
}

// buildBunkerHash creates a hash string to detect state changes
func buildBunkerHash(bunkerMap map[string]*BunkerLog, names []string) string {
	var b strings.Builder
	for _, name := range names {
		l := bunkerMap[name]
		b.WriteString(fmt.Sprintf("%s:%s:%s;", name, l.Status, l.ActivatedAgo))
	}
	return b.String()
}

func sendBunkerEmbed(logs []*BunkerLog) error {
	if len(logs) == 0 {
		return nil
	}
	if Config.BunkerChannelID == "" {
		return fmt.Errorf("BUNKER_CHANNEL_ID not configured")
	}

	// Group by bunker name
	// Use status from Active/Locked line, but keep time from Activated line (has real elapsed time)
	bunkerMap := make(map[string]*BunkerLog)
	for _, l := range logs {
		existing, exists := bunkerMap[l.BunkerName]
		if !exists {
			bunkerMap[l.BunkerName] = l
		} else if existing.Status == "Activated" && (l.Status == "Active" || l.Status == "Locked") {
			// Keep the real elapsed time from the Activated line
			l.ActivatedAgo = existing.ActivatedAgo
			bunkerMap[l.BunkerName] = l
		}
	}

	// Sort bunker names
	bunkerNames := make([]string, 0, len(bunkerMap))
	for name := range bunkerMap {
		bunkerNames = append(bunkerNames, name)
	}
	sort.Strings(bunkerNames)

	// Check if state changed - skip edit if same
	newHash := buildBunkerHash(bunkerMap, bunkerNames)
	if newHash == lastBunkerHash {
		return nil // No change, skip update
	}

	// Build elegant minimal embed
	activeCount := 0
	totalCount := len(bunkerNames)

	var desc strings.Builder
	desc.WriteString("\n")

	for _, name := range bunkerNames {
		l := bunkerMap[name]
		...
		desc.WriteString(fmt.Sprintf("%s **%s Bunker**\n", icon, name))
		desc.WriteString(fmt.Sprintf("\u2003\u2003\u2003%s \u2022 `%s`\n\n", statusText, l.ActivatedAgo))
	}

	// Summary line
	desc.WriteString(fmt.Sprintf("\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\n"))
	desc.WriteString(fmt.Sprintf("\u2003\u2003\u2003\u2003\u2003**%d** / **%d** Active\n", activeCount, totalCount))
	...
	bunkerMsgMux.RLock()
	msgID := bunkerMessageID
	bunkerMsgMux.RUnlock()

	if msgID != "" {
		_, err := SharedSession.ChannelMessageEditEmbed(Config.BunkerChannelID, msgID, embed)
		if err == nil {
			// Edit successful
			lastBunkerHash = newHash
			log.Printf("[BUNKER] Updated embed (%d bunkers, %d active)", totalCount, activeCount)
			UpdateSharedActivity()
			return nil
		}
		// Edit failed (message deleted?) - send new one
		log.Printf("[BUNKER] Edit failed, sending new embed: %v", err)
	}

	// Send new message
	msg, err := SharedSession.ChannelMessageSendEmbed(Config.BunkerChannelID, embed)
	if err != nil {
		log.Printf("ERROR: Failed to send bunker embed: %v", err)
		return err
	}

	// Save message ID for future edits
	saveBunkerMessageID(msg.ID)
	lastBunkerHash = newHash
	log.Printf("[BUNKER] Sent new embed (ID: %s, %d bunkers, %d active)", msg.ID, totalCount, activeCount)
	UpdateSharedActivity()
	return nil
}
...
func processGameplayLines(lines []string) {
	...
	// Collect bunker logs for live status embed (no cache - always read)
	var bunkerLogs []*BunkerLog

	for _, line := range lines {
		// ✅ Collect bunker logs (no cache - live status)
		if strings.Contains(line, "[LogBunkerLock]") {
			info := parseBunkerLog(line)
			if info != nil {
				bunkerLogs = append(bunkerLogs, info)
			}
			continue // Bunker logs are separate from gameplay logs
		}
	...
	// Send/update bunker status embed (live - edit existing message)
	if len(bunkerLogs) > 0 {
		if err := sendBunkerEmbed(bunkerLogs); err != nil {
			log.Printf("Error sending bunker embed: %v", err)
		} else {
			bunkerCount = len(bunkerLogs)
		}
	}

	log.Printf("DEBUG: Parsed %d new gameplay logs, %d cached, %d trap events, %d bunker events", parsedCount, cachedCount, trapCount, bunkerCount)
}
