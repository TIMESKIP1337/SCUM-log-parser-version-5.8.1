...
type SharedConfig struct {
	...
	FakePlayerMin              int
	FakePlayerMax              int
	FakePlayerChangeInterval   int // seconds
	PlayersOnlineCheckInterval int // seconds (dedicated interval)
...
func init() {
	...
	Config = SharedConfig{
		...
		BunkerChannelID: getEnv("BUNKER_CHANNEL_ID", ""),
		...
		MaxPlayerSlot:              getEnvInt("MAX_PLAYER_SLOT", 64),
		FakePlayerMin:              getEnvInt("FAKE_PLAYER_MIN", 0),
		FakePlayerMax:              getEnvInt("FAKE_PLAYER_MAX", 0),
		FakePlayerChangeInterval:   getEnvInt("FAKE_PLAYER_CHANGE_INTERVAL", 900),
		PlayersOnlineCheckInterval: getEnvInt("PLAYERS_ONLINE_CHECK_INTERVAL", 10),
...
func logConfiguration() {
	...
	log.Printf("   Players Online Channel ID: %s", Config.PlayersOnlineChannelID)
}
...
func isPlayersOnlineModuleActive() bool {
	return Config.PlayersOnlineChannelID != "" && Config.ScumLogPath != ""
}
...
func Run() {
	...
	// Initialize players online module if configured
	if isPlayersOnlineModuleActive() {
		log.Println("👥 Initializing Players Online module...")
		if err := InitializePlayersOnline(); err != nil {
			log.Printf("⚠️ Players Online module initialization failed (will continue): %v", err)
		}
	}
...
func logActiveModules() {
	...
	if isPlayersOnlineModuleActive() {
		log.Printf("   👥 Players Online Module: ACTIVE (Channel: %s, Interval: %ds)", Config.PlayersOnlineChannelID, Config.PlayersOnlineCheckInterval)
		log.Printf("      └─ SCUM Log: %s, MaxSlot: %d, Fake: %d-%d", Config.ScumLogPath, Config.MaxPlayerSlot, Config.FakePlayerMin, Config.FakePlayerMax)
	}
